...
export default function RegisterPage() {
  ...
  const schema = role === 'company' ? companySchema : baseSchema;
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800">
      ...
      >
        <h1 className="text-2xl font-bold text-center text-gray-900 dark:text-white mb-1">Create your account</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-6">Join thousands on InternHub</p>

        {/* Role selector */}
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            { value: 'student', icon: GraduationCap, label: 'Student', desc: 'Find internships' },
            { value: 'company', icon: Building2, label: 'Company', desc: 'Post jobs' },
            ...
            <button
              ...
              className={cn(
                ...
                role === value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'
              )}
            >
              <Icon className={cn('h-6 w-6', role === value ? 'text-blue-600' : 'text-gray-400')} />
              <span className={cn('text-sm font-semibold', role === value ? 'text-blue-700 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300')}>{label}</span>
              <span className="text-xs text-gray-400">{desc}</span>
            </button>
          ))}
        </div>

        {/* Google OAuth — students only (companies need company_name so use email flow) */}
        {role === 'student' && (
          <>
            <button
            ...
            <div className="relative mb-4">
              ...
              <div className="relative flex justify-center text-xs text-gray-400 bg-white dark:bg-gray-800 px-2">or register with email</div>
            </div>
          </>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {role === 'company' && (
            <Input label="Company name" placeholder="Acme Corp" error={errors.company_name?.message} {...register('company_name')} />
          )}
